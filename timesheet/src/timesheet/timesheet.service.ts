  ...
  async addTimesheet(data: CreateTimesheetDto): Promise<any> {
    ...
    if (!uuidRegex.test(data.costId)) {
      throw new HttpException({
        ...
        details: {
          ...
          expectedFormat: 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx (where x is any hexadecimal digit and y is one of 8, 9, a, or b)',
    ...
    for (const day of data.days) {
      ...
      try {
        ...
        if (timesheet.day_type === 'working') totalWorkingHours += timesheet.workingHours;
  ...
  async getTimesheetByYear(year: number, costId: string): Promise<any> {
    ...
    if (!timesheets.length) {
      throw new NotFoundException({
        ...
        details: 'Please verify the year and costId provided, or create new timesheet entries for this period.'
      });
    }

    const totalVacation = timesheets.filter(t => t.day_type === 'vacation').length;
    ...
    timesheets.forEach(timesheet => {
      ...
      if (timesheet.day_type === 'vacation') monthlyData[month].total_vacation_leaves++;
      if (timesheet.day_type === 'sick') monthlyData[month].total_sick_leaves++;
      if (timesheet.day_type === 'working') monthlyData[month].total_working_hours += timesheet.workingHours;
    ...
    for (const month of monthNames) {
      monthlyData[month].days.sort((a, b) => a.date.getTime() - b.date.getTime());
    }
    ...
    for (const update of updates) {
      if (update.dayType === 'sick' && update.workingHours > 0) {
        throw new HttpException({
          ...
          details: 'When updating to sick leave, please set working hours to 0.'
        }, HttpStatus.BAD_REQUEST);
      }
      ...
      if (update.dayType === 'vacation' && update.workingHours > 0) {
        throw new HttpException({
          ...
          details: 'When updating to vacation, please set working hours to 0.'
        }, HttpStatus.BAD_REQUEST);
      }
      ...
      if (update.dayType === 'working' && update.workingHours === 0) {
        throw new HttpException({
          ...
          details: 'When updating to a working day, please specify working hours greater than 0.'
        }, HttpStatus.BAD_REQUEST);
      }
      ...
      if (!timesheet) {
        throw new NotFoundException({
          ...
          details: 'Please verify the date or create a new timesheet entry for this date.'
        });
      }
